...
#define FF_ALLOC_EVENT   (SDL_USEREVENT + 10)
#define FF_REFRESH_EVENT (SDL_USEREVENT + 11)
#define FF_QUIT_EVENT    (SDL_USEREVENT + 12)
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    if (!is)
        return NULL;
    strncpy(is->filename, filename, 4095);
    is->filename[4095] = 0;
    is->iformat = iformat;
    if (screen) {
        is->width = screen->w;
...
int ffplay_open(char *filename) {
    if (cur_stream) {
        ffplay_close(cur_stream);
    }

    screen = SDL_GetVideoSurface();
    
    cur_stream = stream_open(filename, NULL);        
    if (!cur_stream) {
        return 1;
    } else {
        return 0;
...
int ffplay_event_handler(int type) {
