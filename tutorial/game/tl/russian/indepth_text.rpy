...
translate russian text_72fb5a9a:

    # e "For example, this displays the [variable]." ### variable not translating
    e "Например, так мы отобразим [variable]."
...
translate russian text_15bfae8e:
    ...
    e "Когда переменная дополняется !q, специальные символы не исключаются. Это позволяет показать чистый [variable!q] вместе с курсивными тегами." ### склонение

# game/indepth_text.rpy:118
translate russian text_c90f24a8:

    # e "When the variable name is followed by !t, it is translated to [variable!t]. It could be something else in a different language." ### здесь должно быть translatable!t ?
    e "Когда переменная дополняется !t, она переводится в [variable!t]. Для других языков эта фраза может звучать по-другому."
