...
translate korean tutorial_video_f34a17f5:
    ...
    e "렌'파이는 동영상 재생을 지원합니다. 동영상을 추가하는 데에는 두가지 방법이 있는데요."
...
translate korean tutorial_video_b927d009:
    ...
    e "이렇게 하기 위해서는, 먼저 이미지를 Movie 디스플레이어블으로 정의해야 합니다. Movie 디스플레이어블은 필수적으로 크기와 스크린 위의 위치를 결정하는 속성들을 인수로 받아야 합니다."


# game/tutorial_video.rpy:28
translate korean tutorial_video_d4860780:


    # e "Then, we can show the movie displayable, and start the movie playing with a play statement."
    e "이제 Movie 디스플레이어블을 사용할 수 있어요! play 문을 사용해서 재생을 시작할 수 있습니다."


# game/tutorial_video.rpy:37
translate korean tutorial_video_373e48d8:


    # e "When we no longer want to play the movie, we can stop it, and then hide it."
    e "동영상을 더 이상 재생하고 싶지 않을 때는 재생을 멈추고 숨길 수 있어요."
...
translate korean tutorial_video_a66b154c:
    ...
    e "동영상을 표시하는 다른 방법은 renpy.movie_cutscene 파이썬 함수를 사용하는 것입니다. 이 함수는 영상이 끝나거나 플레이어가 클릭할 때까지 영상을 전체화면으로 재생합니다."


# game/tutorial_video.rpy:49
translate korean tutorial_video_7b2dc95f:


    # e "And that's all there is when it comes to movie playback in Ren'Py."
    e "그리고 이게 비디오 재생하기 단원의 마지막이에요."