...
translate korean tutorial_video_b56ccf19:
    ...
    e "이렇게 하려면 먼저 이미지(image)를 무비(Movie) 디스플레이어블로 정의해야 해. 무비 디스플레이어블은 재생하는 영상에 위치 속성을 부여할 수 있어."

# game/tutorial_video.rpy:25
translate korean tutorial_video_fbaa71e4:

    # e "Then, we can show the movie displayable, which starts the movie playing."
    e "그러면, 우리는 재생되는 무비 디스플레이어블을 보여줄 수 있어."

# game/tutorial_video.rpy:30
translate korean tutorial_video_bbbb242d:

    # e "When we no longer want to play the movie, we can hide it."
    e "더이상 보여주고 싶지 않을 때는 hide로 영상을 제거하면 돼."

# game/tutorial_video.rpy:34
translate korean tutorial_video_a66b154c:
    ...
    e "영상을 보여주는 또 다른 방법은 파이썬 함수 renpy.movie_cutscene을 사용하는 거야. 이 함수는 영상이 끝나거나 사용자가 클릭할 때까지 전체 화면에 영상을 표시해."

# game/tutorial_video.rpy:41
translate korean tutorial_video_63e75209:
    ...
    e "무비 디스플레이어블은 알파 채널로 마스크를 사용해서 무비 스프라이트를 만들 수도 있어. 하지만 그건 좀 더 복잡하니까, 지금은 여기서 그만하도록 하자."
