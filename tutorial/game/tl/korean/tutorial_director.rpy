...
translate korean director_ebf40500:
    ...
    e "Shift+R을 입력하면 자동읽기 모드가 활성화돼. 이 기능을 사용하면 스크립트 파일을 편집할 때 게임이 자동으로 다시 로드돼."

# game/tutorial_director.rpy:9
translate korean director_6f3d1bea:

    # e "Shift+O brings you to the console, which lets you enter Ren'Py and Python commands to try them out."
    e "Shift+O를 누르면 렌파이와 파이썬 명령을 입력하여 콘솔을 사용해볼 수 있어."

# game/tutorial_director.rpy:11
translate korean director_70a61c1c:

    # e "Shift+D pops up a developer menu with access to these and other functions."
    e "Shift+D는 이 기능과 다른 기능에 액세스할 수 있는 개발자 메뉴를 표시해."

# game/tutorial_director.rpy:13
translate korean director_43504744:
    ...
    e "가장 강력한 도구는 렌파이 내부에서 게임에 이미지, 음악 및 음성을 추가할 수 있는 인터렉티브 디렉터야."

# game/tutorial_director.rpy:15
translate korean director_32f8695e:
    ...
    e "편집기를 사용하여 시각 소설의 스크립트와 논리를 작성한 다음 대화식으로 적절한 위치에 이미지를 추가할 수 있게 만들어졌어."

# game/tutorial_director.rpy:21
translate korean director_62734181:
    ...
    e "렌파이가 시스템에 읽기 전용으로 설치되어 있기 때문에 지금은 인터렉티브 디렉터를 사용해 볼 수 없어."
...
translate korean director_453d4d67:
    ...
    e "이 길라잡이 게임을 변경하기 위해 인터렉티브 디렉터를 지금 사용할 수 있어."

# game/tutorial_director.rpy:31
translate korean director_c35284d1:

    # e "Be sure to click my dialogue at the bottom of the screen to advance the tutorial."
    e "길라잡이를 진행하려면 화면 아래 내 대사를 클릭해봐."

# game/tutorial_director.rpy:33
translate korean director_e253e03b:
    ...
    e "문제가 생기더라도 걱정하지 마. 이 길라잡이를 종료하고 다시 시작하면 변경 사항이 제거되고 모든 항목은 다시 원래의 상태로 돌아가."

# game/tutorial_director.rpy:42
translate korean director_1cd735bc:

    # e "To get started, let's go back to a blank slate, with no images on the screen."
    e "시작하려면 화면에 이미지가 없는 빈 슬레이트로 돌아가."

# game/tutorial_director.rpy:44
translate korean director_c7a18979:
    ...
    e "키보드의 'D' 키를 눌러 언제든지 디렉터를 표시할 수 있어. 렌파이가 다시 로드되고, 여기로 돌아올거야. 지금 실험해봐."

# game/tutorial_director.rpy:46
translate korean director_3dcc4362:

    # e "Let's add a background. Click the '+' to pick where to add it, then the 'scene' statement and 'washington' for the image. Finally, click '추가' to add it."
    e "이제 배경을 추가해보자. 추가할 위치를 선택하려면 '+'를 클릭하고 이미지의 경우 'scene' 문과 'washington'을 클릭해. 마지막으로 '추가'를 클릭하면 돼."

# game/tutorial_director.rpy:48
translate korean director_292d58b5:

    # e "Next, add a sprite. Click '+', then 'show', 'eileen', 'happy', and '추가'. Once you've added it, dissolve it in by clicking the second '+', then 'with', 'dissolve', and '추가'."
    e "다음은 스프라이트를 추가하자. '+', 'show', 'eileen', 'happy'및 '추가'를 클릭해. 추가한 후에는 두 번째 '+', 'with', 'dissolve'및 '추가'를 클릭하여 디졸브 효과를 넣어봐."
...
translate korean director_4e04a74e:
    ...
    e "마지막으로, play, queue, stop 및 voice 명령문을 사용하여 오디오를 관리할 수 있어. 'play', 'music', 'sunflower-slow-drag.ogg'를 추가해봐."
...
translate korean director_c6dd0c81:
    ...
    e "디럭터로 변경한 사항은 영구적이야. 스크립트에 저장되며 이 구간을 이전으로 돌리거나 반복하여 볼 수 있어."
