...
translate korean demo_transitions_5bbc72fe:
    ...
    e "렌'파이는 아주 많은 내장 트랜지션을 가지고 있지만, 여러분의 트랜지션을 직접 정의할 수 있도록 여러 클래스를 포함하고 있어요."
...
translate korean demo_transitions_menu_3caf78d3:
    ...
    e "어떤 형식의 트랜지션을 보고 싶으세요?" nointeract
...
translate korean demo_simple_transitions_2b4fbae3:
    ...
    e "좋아요. 그러면 단순 트랜지션에 대해 이야기를 시작해 볼까요? 이 종류의 트랜지션은 수정할 수 있는 옵션을 많이 주지 않기 때문에 \"단순\" 트랜지션이라고 불립니다."
...
translate korean demo_simple_transitions_4b235ac2:
    ...
    e "아, 그래도 실망하지는 마세요. 이 트랜지션이 여러분이 게임에서 가장 많이 사용할 트랜지션일 테니까요."
...
translate korean demo_simple_transitions_af0431ac:
    ...
    e "아마 디졸브 트랜지션이 가장 유용할 거라고 생각되는데요, 이 트랜지션은 어떤 장면을 다른 장면과 혼합하면서 화면을 전환합니다."
...
translate korean demo_simple_transitions_5b9f711f:
    ...
    e "Dissolve 함수는 여러분이 직접 다른 시간을 가지는 디졸브 트랜지션을 만들 수 있게 합니다. "
...
translate korean demo_simple_transitions_79816523:
    ...
    e "fade 트랜지션은 검정색 화면으로 페이드 후, 새로운 장면으로 페이드 합니다."
...
translate korean demo_simple_transitions_141bb95d:
    ...
    e "만약 검은 화면 상태로 만들고 싶다면 fade 대신 dissolve를 사용하는 것이 좋아요."
...
translate korean demo_simple_transitions_f059f4ae:
    ...
    e "Fade 함수를 사용하면 여러분만의 페이드를 만들 수 있어요. 페이드하는데 걸릴 시간과 페이드 될 색상을 인수로 넘겨주면 카메라 플래시와 같은 효과를 만들 수 있어요."
...
translate korean demo_simple_transitions_e948905b:
    ...
    e "pixellate 트랜지션은 이전 장면을 서서히 모자이크 처리 후 새로운 장면으로 바꾸고, 서서히 모자이크를 없앱니다."
...
translate korean demo_simple_transitions_6a1ae05f:
    ...
    e "많은 게임에게는 어울리지 않는 트랜지션이지만, 꽤 멋지다고는 생각해요."
...
translate korean demo_simple_transitions_bdfcd85a:
    ...
    e "Pixellate 함수를 사용하면 이 트랜지션의 몇가지 세부사항을 조정할 수 있어요."
...
translate korean demo_simple_transitions_51c1c5b8:
    ...
    e "...아니에요, 그냥 vpunch를 시연해 보려던 것 뿐이었어요."
...
translate korean demo_simple_transitions_57f19473:
    ...
    e "hpunch를 사용하면 스크린을 가로 방향으로 흔들 수 있어요. 이러한 트랜지션은 Move 함수를 통해 정의되었답니다."
...
translate korean demo_simple_transitions_fce83e12:
    ...
    e "move 트랜지션도 존재합니다만, 헷갈리게도 이 트랜지션은 MoveTransition 함수로 선언되었습니다."
...
translate korean demo_simple_transitions_fbb1838e:


    # e "Finally, there's 'Pause', which lets you define a transition that just waits for a given amount of time."
    e "마지막으로, 주어진 시간만큼 기다리는 Pause 트랜지션이 있습니다."


# game/demo_transitions.rpy:145
translate korean demo_simple_transitions_654a7a87:
...
translate korean demo_imagedissolve_transitions_2db67018:
    ...
    e "아마 가장 유연한 트랜지션은 ImageDissolve, 디졸브를 이미지를 통해 컨트롤하는 트랜지션일 거예요."
...
translate korean demo_imagedissolve_transitions_429f8d03:
    ...
    e "이는 여러분이 굉장히 복잡한 트랜지션을 비교젹 간단하게 사용할 수 있게 해 줍니다. 몇 가지 예제를 볼까요? 그 다음 어떻게 이러한 것들이 동작하는지 알려 드릴게요."
...
translate korean demo_imagedissolve_transitions_1ce501b0:
    ...
    e "렌'파이는 두개의 ImmageDissolve 트랜지션을 내장하고 있습니다."
...
translate korean demo_imagedissolve_transitions_ea0413be:
    ...
    e "blinds 트랜지션은 세로로 블라인드를 치는 것 같은 효과를 줍니다."
...
translate korean demo_imagedissolve_transitions_bbf73d1c:
    ...
    e "저는 왜 사람들이 이런걸 사용하기를 원하는지는 모르겠지만 몇개의 번역된 게임에서 사용되었으므로 추가를 했어요."
...
translate korean demo_imagedissolve_transitions_54aa9bf9:
    ...
    e "왜냐하면 이러한 트랜지션들은 스크린의 크기를 요구하는데요, 그렇기 때문에 게임마다 스크린 크기가 달라질 가능성이 있어서 넣을 수 없었습니다."


# game/demo_transitions.rpy:198
translate korean demo_imagedissolve_transitions_70cfd5ed:


    # e "You can click the button above to see how they are defined in the demo script."
    e "그럼 데모 스크립트에서 이러한 트랜지션들이 어떻게 정의되었는지 볼까요?"
...
translate korean demo_imagedissolve_transitions_b8fdf2b6:
    ...
    e "circleirisout으로 다시 보여줄 수 있습니다."
...
translate korean demo_imagedissolve_transitions_ee427486:
    ...
    e "circlewipe 트랜지션은 원 모양의 닦아내기 효과로 스크린을 바꿉니다."
...
translate korean demo_imagedissolve_transitions_6f089276:
    ...
    e "이 dream 트랜지션은 좀 이상한 구불구불한 디졸브를 상대적으로 느리게 실행합니다."
...
translate korean demo_imagedissolve_transitions_c0b9d74d:
    ...
    e "teleport 트랜지션은 새로운 장면을 한번에 한줄씩 바꿔나갑니다."
...
translate korean demo_imagedissolve_transitions_72ba11d4:
    ...
    e "그리고 이게 circleirisout 트랜지션에 사용된 배경 그림입니다."
...
translate korean demo_imagedissolve_transitions_fc3b3339:
    ...
    e "ImageDissolve를 사용하면 이미지의 하얀 부분이 먼저 디졸브되고 그 다음으로 밝은 순서대로 디졸브됩니다. 다음과 같이요."
...
translate korean demo_imagedissolve_transitions_4327dca2:
    ...
    e "만약 제가 ImageDissolve에 reverse 인수를 넘겨주면, 검정색 부분이 먼저 디졸브 되겠죠."
...
translate korean demo_imagedissolve_transitions_3a401ee7:
    ...
    e "이 기능이 circleirisin과 circleirisout 트랜지션을 같은 이미지로 구현할 수 있게 해 주었습니다."
...
translate korean demo_imagedissolve_transitions_20d9cf6c:
    ...
    e "transport 트랜지션은 다른 이미지를 사용하는데요, 이 트랜지션은 한번에 한줄씩 어떤 것을 디졸브합니다."
...
translate korean demo_imagedissolve_transitions_4b557a29:
    ...
    e "...이런 식으로 캐릭터에게만 텔레포트 효과를 적용시킬 수 있죠."


# game/demo_transitions.rpy:257
translate korean demo_imagedissolve_transitions_e16f8d3a:


    # e "For more examples of ImageDissolve, check out the {i}Utsukushii Effects{/i} demo."
    e "ImageDissolve에 관한 더 많은 예시를 보려면 {i}Utsukushii Effects{/i} 데모를 봐 주세요."


# game/demo_transitions.rpy:259
translate korean demo_imagedissolve_transitions_b6dbf68b:


    # e "It shows how a clever game-maker can use ImageDissolve to create all sorts of effects."
    e "이는 영리한 게임 제작자가 어떻게 ImageDissolve를 사용해서 이러한 트랜지션을 만드는지 잘 보여주니까요."
...
translate korean demo_cropmove_transitions_1711a91e:
    ...
    e "CropMove 트랜지션 클래스는 넓은 범위의 트랜지션 효과를 제공합니다. 하지만 잘 사용되지 않기는 하죠."
...
translate korean demo_cropmove_transitions_6d82cfd7:
    ...
    e "그럼 여러분이 트랜지션 효과를 볼 수 있게 저는 잠시 스크린 밖에 나가 있을게요. 트랜지션의 이름은 트랜지션이 나온 다음에 읽어 드릴게요."
...
translate korean demo_cropmove_transitions_b8a710c1:
    ...
    e "슬라이드 트랜지션이 새로운 장면을 슬라이드한다면, slideaway 트랜지션은 이전 장면을 슬라이드해 내보냅니다."
...
translate korean demo_cropmove_transitions_025ef723:
    ...
    e "또 직사각형 모양의 조리개 효과를 사용하는 트랜지션도 몇가지 있습니다."
...
translate korean demo_cropmove_transitions_08d753ed:
    ...
    e "여러분을 좀 어지럽게 하는데 이정도면 충분할 듯 싶네요."
...
translate korean demo_movetransition_14df0e34:
    ...
    e "가장 일반적인 MoveTransition은 move인데요, 이 트랜지션은 위치가 바뀐 이미지를 그저 슬라이드해서 이동시킵니다."
...
translate korean demo_movetransition_098ee9f1:
    ...
    e "또 moveout과 movein 트랜지션이 있습니다."
...
translate korean demo_movetransition_09748f81:
    ...
    e "moveout 트랜지션들(moveoutleft, moveoutright, moveouttop, moveoutbottom)은 이미지를 주어진 방향으로 슬라이드해서 숨깁니다."
...
translate korean demo_movetransition_5edf6007:
    ...
    e "movein 트랜지션들(moveinleft, moveinright, moveintop, moveinbottom)은 새 이미지를 슬라이드해서 보여주구요."
...
translate korean demo_movetransition_569952e3:
    ...
    e "movein과 moveout에 관해서는 이게 끝입니다."


# game/demo_transitions.rpy:398
translate korean demo_movetransition_af4c53cf:


    # e "Finally, there are the zoomin and zoomout transtions, which show and hide things using a zoom."
    e "마지막으로, zoomin과 zoomout 트랜지션이 있는데요, 이들은 이미지를 확대/축소해서 이미지를 보이거나 숨깁니다."
...
translate korean demo_alphadissolve_3efbde9f:


    # e "The AlphaDissolve transition lets you use one displayable to combine two others. For example..."
    e "AlphaDissolve 트랜지션은 두개의 디스플레이어블을 하나의 디스플레이어블으로 합칠 수 있게 해 줍니다. 예를 들어..."
...
translate korean demo_alphadissolve_7c08cf8b:
    ...
    e "AlphaDissolve 디스플레이어블은 컨트롤 디스플레이어블을 받는데요, 대개 ATL 트랜스폼입니다."
...
translate korean demo_alphadissolve_068e3e98:
    ...
    e "이게 유용히 사용되려면 컨트롤 디스플레이어블은 부분적으로 투명해야 할 필요가 있습니다."
...
translate korean demo_alphadissolve_6a1b6203:
    ...
    e "AlphaDissolve 도중에, 이전 스크린이 이미지의 투명한 부분을 채우고 새로운 스크린이 불투명한 부분을 채워 넣습니다."
...
translate korean demo_alphadissolve_80a728b6:
    ...
    e "방금 전의 스포트라이트 예제에서 이전 스크린은 완전히 까만 이미지입니다."
...
translate korean demo_alphadissolve_2e95917b:
    ...
    e "AlphaDissolve를 사용해 이 둘을 결합시키면 간단한 부분을 이용해서 복잡한 효과를 만들 수 있습니다."
...
translate korean strings:
    ...
    new "ImageDissolve 트랜지션"


    # game/demo_transitions.rpy:46
    old "MoveTransition Transitions"
    new "MoveTransition 트랜지션"
