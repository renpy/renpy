
# game/tutorial_nvlmode.rpy:17
translate korean tutorial_nvlmode_76b2fe88:

    # nvl clear
    nvl clear

# game/tutorial_nvlmode.rpy:20
translate korean tutorial_nvlmode_ac125210:

    # nvle "NVL-style games are games that cover the full screen with text, rather than placing it in a window at the bottom of the screen. Like this."
    nvle "NVL 스타일 게임은 화면 아래쪽에 창을 표시하지 않고 전체 화면을 글자로 채우는 게임이야. 이렇게 말야."

# game/tutorial_nvlmode.rpy:24
translate korean tutorial_nvlmode_e7030787:

    # nvle "To use NVL-mode, you need to define Characters with a kind=nvl."
    nvle "NVL 모드를 사용하려면, 캐릭터(Character)를 kind=nvl로 정의(define)해야 해."

# game/tutorial_nvlmode.rpy:28
translate korean tutorial_nvlmode_c9a35125:

    # nvle "Then just use that character in a say statement."
    nvle "그다음 say 명령문으로 캐릭터를 사용하기만 하면 돼."

# game/tutorial_nvlmode.rpy:30
translate korean tutorial_nvlmode_130610c2:

    # nvl clear
    # nvle "You use 'nvl clear' to clear the screen when that becomes necessary."
    nvl clear
    nvle "화면을 지워야 할 때는 'nvl clear'를 사용하면 돼."

# game/tutorial_nvlmode.rpy:37
translate korean tutorial_nvlmode_390a4eb1:

    # nvle "The 'nvl show' and 'nvl hide' statements use transitions to show and hide the NVL window."
    nvle "'nvl show'와 'nvl hide' 명령문은 NVL 창을 보이거나 숨길 때 사용해."

# game/tutorial_nvlmode.rpy:48
translate korean tutorial_nvlmode_05956e33:

    # nvle "NVL-mode also supports showing menus to the player, providing it's the last thing on the screen. Understand?" nointeract
    nvle "NVL 모드는 선택지도 지원해. 스크린의 마지막에 선택지를 출력할 수 있어." nointeract

# game/tutorial_nvlmode.rpy:54
translate korean tutorial_nvlmode_0f2b7d59:

    # nvl clear
    # nvle "Good!"
    nvl clear
    nvle "좋아!"

# game/tutorial_nvlmode.rpy:60
translate korean tutorial_nvlmode_f3a79c09:

    # nvl clear
    # nvle "Well, hopefully the code below makes it a little more clear."
    nvl clear
    nvle "음, 아래 코드가 좀 더 명확한 설명이 될 거야."

# game/tutorial_nvlmode.rpy:69
translate korean after_nvl_menu_cb560cd2:

    # nvle "Games can mix NVL-mode and the normal ADV-mode by having some characters that have kind=nvl, and some that do not."
    nvle "게임은 kind = nvl인 캐릭터와 그렇지 않은 캐릭터를 사용해 NVL 모드와 일반 ADV 모드를 혼합할 수 있어."

# game/tutorial_nvlmode.rpy:71
translate korean after_nvl_menu_10a6e85a:

    # e "You can specify transitions that occur when going from NVL-mode to ADV-mode."
    e "NVL 모드에서 ADV 모드로 전환할 때 보여질 전환(transition)을 지정할 수 있어."

# game/tutorial_nvlmode.rpy:73
translate korean after_nvl_menu_d43b28d1:

    # nvle "As well as when going from ADV-mode to NVL-mode."
    nvle "ADV 모드에서 NVL 모드로 전환할 때도 마찬가지고."

# game/tutorial_nvlmode.rpy:75
translate korean after_nvl_menu_f056c7ad:

    # nvle "Text tags like {{w}{w} work in NVL-mode."
    nvle "{{w}{w} 같은 텍스트 태그들도 NVL 모드에서 작동해."

# game/tutorial_nvlmode.rpy:77
translate korean after_nvl_menu_750cd9a1:

    # extend " As does the \"extend\" special character."
    extend "\"extend\" 특수 캐릭터가 그렇듯이."

# game/tutorial_nvlmode.rpy:79
translate korean after_nvl_menu_146d840b:

    # nvle "And that's it for NVL-mode."
    nvle "그리고 그건 NVL 모드 용이야."

translate korean strings:

    # tutorial_nvlmode.rpy:48
    old "Yes."
    new "네"

    # tutorial_nvlmode.rpy:48
    old "No."
    new "아니오"

