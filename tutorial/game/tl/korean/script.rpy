

# game/script.rpy:85
translate korean start_0e6a5bb4:


    # e "Hi! My name is Eileen, and I'd like to welcome you to the Ren'Py tutorial."
    e "안녕하세요, 아이린입니다! 렌'파이 길라잡이에 오신것을 환영해요!"


# game/script.rpy:89
translate korean start_d3abb53c:


    # e "In this tutorial, we'll teach you the basics of Ren'Py, so you can make games of your own. We'll also demonstrate many features, so you can see what Ren'Py is capable of."
    e "이 길라잡이에서는 여러분이 직접 게임을 만들 수 있도록 렌'파이의 기본을 가르쳐드릴 거예요! 또한, 렌'파이가 무엇을 할 수 있는지 시범도 보여드리겠습니다."


# game/script.rpy:118
translate korean end_b2482727:


    # e "Thank you for viewing this tutorial."
    e "이 길라잡이를 봐주셔서 감사합니다."


# game/script.rpy:120
translate korean end_38362e36:


    # e "If you'd like to see a full Ren'Py game, select \"The Question\" in the launcher."
    e "실제 렌'파이 게임을 보고 싶으시다면 런처에서 \"The Question\"을 선택해 주시기 바랍니다."


# game/script.rpy:122
translate korean end_894a9e66:


    # e "You can download new versions of Ren'Py from {a=http://www.renpy.org/}http://www.renpy.org/{/a}. For help and discussion, check out the {a=http://lemmasoft.renai.us/forums/}Lemma Soft Forums{/a}."
    e "새로운 렌'파이 버전은 {a=http://www.renpy.org/}http://www.renpy.org/{/a}에서 다운로드하실 수 있습니다! 도움이나 토론을 원하신다면, {a=http://lemmasoft.renai.us/forums/}렘마 소프트 포럼{/a}을 확인해 주세요."


# game/script.rpy:124
translate korean end_a898b369:


    # e "We'd like to thank Piroshki for contributing my sprites, Mugenjohncel for Lucy and the band, and Jake for the magic circle."
    e "제 스프라이트를 제공해 주신 Piroshki님, 루시와 밴드를 그려 주신 Mugenjohncel님, Magic Circle을 제공해주신 Jake님께 감사를 드립니다."


# game/script.rpy:126
translate korean end_762dc07a:


    # e "The background music is \"Sunflower Slow Drag\", by Scott Joplin and Scott Hayden, performed by the United States Marine Band. The concert music is by Alessio."
    e "배경음악은 미합중국 해군 군악대가 연주한 Scott Joplin과 Scott Hayden의 \"Sunflower Slow Drag\"입니다. 콘서트 음악은 Alessio님의 작품입니다."


# game/script.rpy:130
translate korean end_57c21073:


    # e "We look forward to seeing what you can make with Ren'Py. Have fun!"
    e "렌'파이와 함께하는 당신의 가능성, 기대하고 있겠습니다. 좋은 시간 보내세요!"


translate korean strings:


    # game/script.rpy:18
    old "User Experience"
    new "렌'파이의 기본 구조"


    # game/script.rpy:18
    old "Writing Dialogue"
    new "대사 작성하기"


    # game/script.rpy:18
    old "Adding Images"
    new "이미지 추가하기"


    # game/script.rpy:18
    old "Music and Sound Effects"
    new "음악과 효과음"


    # game/script.rpy:18
    old "In-Game Menus and Python"
    new "게임 내 메뉴와 파이썬"


    # game/script.rpy:18
    old "Screen Positions"
    new "스크린 위치 지정"


    # game/script.rpy:18
    old "Animation and Transformation"
    new "애니메이션과 트렌스포메이션"


    # game/script.rpy:18
    old "Video Playback"
    new "비디오 재생하기"


    # game/script.rpy:18
    old "Transition Gallery"
    new "트랜지션 갤러리"


    # game/script.rpy:18
    old "Image Operations"
    new "이미지 조작"


    # game/script.rpy:18
    old "User Interaction"
    new "유저와의 상호작용"


    # game/script.rpy:18
    old "Fonts and Text Tags"
    new "폰트와 텍스트 태그"


    # game/script.rpy:18
    old "Character Objects"
    new "캐릭터 오브젝트"


    # game/script.rpy:18
    old "Layers & Advanced Show"
    new "레이어와 고급 표시"


    # game/script.rpy:18
    old "NVL Mode"
    new "NVL 모드"


    # game/script.rpy:18
    old "Dynamic Displayables"
    new "동적 디스플레이어블"


    # game/script.rpy:18
    old "Minigames"
    new "미니게임"


    # game/script.rpy:18
    old "Persistent Data"
    new "지속 데이터"


    # game/script.rpy:18
    old "Transform"
    new "트랜스폼"


    # game/script.rpy:18
    old "Sprites"
    new "스프라이트"


    # game/script.rpy:64
    old "That's enough for now."
    new "이제 됐어요."


    # game/script.rpy:99
    old "What would you like to see?"
    new "어떤 것 먼저 보실래요?"


    # game/script.rpy:101
    old "Is there anything else you'd like to see?"
    new "다른 것도 보고 싶으세요?"