
# game/script.rpy:156
translate italian start_0e6a5bb4:

    # e "Hi! My name is Eileen, and I'd like to welcome you to the Ren'Py tutorial."
    e "Ciao! Il mio nome è Eileen e vorrei dartti il benvenuto al tutorial di Ren'Py."

# game/script.rpy:160
translate italian start_d3abb53c:

    # e "In this tutorial, we'll teach you the basics of Ren'Py, so you can make games of your own. We'll also demonstrate many features, so you can see what Ren'Py is capable of."
    e "In questo tutorial, ti insegneremo le basi di Ren'Py, così da poter creare i tuoi giochi. Dimostreremo anche molte funzionalità, così da poter vedere di cosa Ren'Py è capace."

# game/script.rpy:205
translate italian end_b2482727:

    # e "Thank you for viewing this tutorial."
    e "Grazie per aver visualizzato questo tutorial."

# game/script.rpy:207
translate italian end_38362e36:

    # e "If you'd like to see a full Ren'Py game, select \"The Question\" in the launcher."
    e "Se vorresti vedere un gioco Ren'Py completo, seleziona \"The Question\" nel launcher."

# game/script.rpy:209
translate italian end_02527d05:

    # e "You can download new versions of Ren'Py from {a=https://www.renpy.org/}https://www.renpy.org/{/a}. For help and discussion, check out {a=https://lemmasoft.renai.us/forums/}Lemma Soft Forums{/a}."
    e "Puoi scaricare nuove versioni di Ren'Py da {a=https://www.renpy.org/}https://www.renpy.org/{/a}. Per aiuto e discussione, dai un'occhiata a {a=https://lemmasoft.renai.us/forums/}Lemma Soft Forums{/a}."

# game/script.rpy:211
translate italian end_c9d03136:

    # e "We'd like to thank Piroshki for contributing my sprites; Mugenjohncel for Lucy, the band, and drawn backgrounds; and Jake for the magic circle."
    e "Vorremmo ringraziare Piroshki per aver contribuito con i miei sprite; Mugenjohncel per Lucy, la banda e gli sfondi disegnati; e Jake per il cerchio magico."

# game/script.rpy:213
translate italian end_762dc07a:

    # e "The background music is \"Sunflower Slow Drag\", by Scott Joplin and Scott Hayden, performed by the United States Marine Band. The concert music is by Alessio."
    e "La musica di sfondo è \"Sunflower Slow Drag\", di Scott Joplin e Scott Hayden, interpretata dalla United States Marine Band. La musica del concerto è di Alessio."

# game/script.rpy:217
translate italian end_a634d396:

    # e "We look forward to seeing what you create with Ren'Py. Have fun!"
    e "Attendiamo di vedere cosa creerai con Ren'Py. Divertiti!"

translate italian strings:

    # script.rpy:11
    old "Eileen"
    new "Eileen"

    # script.rpy:56
    old "Quickstart"
    new "Avvio rapido"

    # script.rpy:58
    old "Player Experience"
    new "Esperienza Giocatore"

    # script.rpy:59
    old "Creating a New Game"
    new "Creazione di un Nuovo Gioco"

    # script.rpy:60
    old "Writing Dialogue"
    new "Scrittura di Dialoghi"

    # script.rpy:61
    old "Adding Images"
    new "Aggiunta di Immagini"

    # script.rpy:62
    old "Positioning Images"
    new "Posizionamento Immagini"

    # script.rpy:63
    old "Transitions"
    new "Transizioni"

    # script.rpy:64
    old "Music and Sound Effects"
    new "Musica e Effetti Sonori"

    # script.rpy:65
    old "Choices and Python"
    new "Scelte e Python"

    # script.rpy:66
    old "Input and Interpolation"
    new "Input e Interpolazione"

    # script.rpy:67
    old "Video Playback"
    new "Riproduzione Video"

    # script.rpy:68
    old "NVL Mode"
    new "Modalità NVL"

    # script.rpy:69
    old "Tools and Interactive Director"
    new "Strumenti e Regista Interattivo"

    # script.rpy:70
    old "Building Distributions"
    new "Costruzione Distribuzioni"

    # script.rpy:72
    old "In Depth"
    new "Approfondimento"

    # script.rpy:74
    old "Text Tags, Escapes, and Interpolation"
    new "Tag di Testo, Escape e Interpolazione"

    # script.rpy:75
    old "Character Objects"
    new "Oggetti Character"

    # script.rpy:76
    old "Simple Displayables"
    new "Visualizzabili Semplici"

    # script.rpy:77
    old "Transition Gallery"
    new "Galleria Transizioni"

    # script.rpy:80
    old "Position Properties"
    new "Proprietà di Posizione"

    # script.rpy:83
    old "Transforms and Animation"
    new "Trasformazioni e Animazione"

    # script.rpy:84
    old "Transform Properties"
    new "Proprietà di Trasformazione"

    # script.rpy:86
    old "GUI Customization"
    new "Personalizzazione GUI"

    # script.rpy:87
    old "Styles and Style Properties"
    new "Stili e Proprietà di Stile"

    # script.rpy:88
    old "Screen Basics"
    new "Fondamenti di Schermo"

    # script.rpy:89
    old "Screen Displayables"
    new "Visualizzabili Schermo"

    # script.rpy:91
    old "Minigames and CDDs"
    new "Minigiochi e CDD"

    # script.rpy:92
    old "Translations"
    new "Traduzioni"

    # script.rpy:129
    old "That's enough for now."
    new "Per ora è sufficiente."

    # script.rpy:168
    old "What would you like to see?"
    new "Cosa vorresti vedere?"

    # script.rpy:170
    old "Is there anything else you'd like to see?"
    new "C'è altro che vorresti vedere?"
# TODO: Translation updated at 2026-04-03 00:05

translate italian strings:

    # game/script.rpy:69
    old "Tools and the Interactive Director"
    new "Strumenti e il Regista Interattivo"

