...
translate russian strings:
    ...
    new "## Строки, начинающиеся  с двух '#' — комментарии, и вы не должен раскомментировать. Строки, начинающиеся с одной '#' — комментированный код, который вы можете раскомментировать, если вы считаете это нужным."
    ...
    new "## Читаемое название игры. Используется в установке стандартного заголовка окна, показывается в интерфейсе и отчётах об ошибках."
    ...
    new "## Эти три переменные контролируют соответствующи микшеры громкости в настройках, которые пользователь может настраивать по умолчанию. Изменив один из параметров на False, скроется соответствующий микшер."
    ...
    new "## После того, как игра началась, это может быть исменено с помощью \"window show\", \"window hide\", и \"window auto\"."
    ...
    new "## Оно обычно не должно изменяться, а если и изменилось, должно быть текстовой строчкой, не выражением."
    ...
    new "## Следующие функции избирают образцы файлов. Образцы файлов не учитывают регистр и соответствующе зависят от директории base, с или без учёта /, задавающей папки. Если берутся многочисленные одноимённые файлы, используется первый."
    ...
    new "## Например, \"*.txt\" берёт все файлы в формате txt из директории base, \"game/**.ogg\" берёт все файлы ogg из директории game и всех поддиректорий, а \"**.psd\" берёт файлы psd из любого места проекта."
    ...
    new "## Файлы соответствующие образцам документации дублируются в приложении mac, так что они появляется и в приложении и в zip архиве."

    # options.rpy:196
    old "## A Google Play license key is required to download expansion files and perform in-app purchases. It can be found on the \"Services & APIs\" page of the Google Play developer console."
    new "## Лицензионный ключ Google Play требуется для загрузки файлов расширений и поддержки внутри-игровых покупок. Он может быть найден на странице \"Services & APIs\" консоли разработчика Google Play."

    # options.rpy:203
    old "## The username and project name associated with an itch.io project, separated by a slash."
    new "## Имя пользователя и название проекта, ассоциированное с проектом на itch.io, разделённые дробью."
