...
translate simplified_chinese strings:
    ...
    new "当前图形驱动可能过期或者配置不正确。这样会导致图形显示缓慢或者错误。升级DirectX可能可以解决该问题。"

    # renpy/common/00gltest.rpy:130
    old "Its graphics drivers may be out of date or not operating correctly. This can lead to slow or incorrect graphics display."
    new "当前图形驱动可能过期或者配置不正确。这样会导致图形显示缓慢或者错误。"
    ...
    new "DirectX在线安装已启动。将会最小化到任务栏中。请按照指示来安装DirectX。"

    # renpy/common/00gltest.rpy:179
    old "{b}Note:{/b} Microsoft's DirectX web setup program will, by default, install the Bing toolbar. If you do not want this toolbar, uncheck the appropriate box."
    new "{b}警告:{/b} Microsoft's DirectX 在线安装程序会默认安装 Bing 浏览器工具栏。如果你不需要改工具栏，请取消勾选对应的选项。"
...
translate simplified_chinese strings:
    ...
    new "尝试回滚在先前的状态，这样你就可以保存或者选择不同的选项。"
...
translate simplified_chinese strings:
    ...
    new "%(version)s 控制台，源自Shiz, C, 以及 delta（构建）。"

    # renpy/common/00console.rpy:180
    old "Press <esc> to exit console. Type help for help.\n"
    new "按<esc>来退出控制台。输入help来获取帮助。\n"
    ...
    new "<renpy script statement>:运行（规定的）状态（statement）\n"

    # renpy/common/00console.rpy:409
    old " <python expression or statement>: run the expression or statement"
    new "<python expression or statement>:运行表达式（expression）或状态（statement）"
    ...
    new "load <slot>:加载游戏存档（以slot为存档号）"

    # renpy/common/00console.rpy:442
    old "save <slot>: saves the game in slot"
    new "save <slot>:存储游戏存档（以slot为存档号）"
    ...
    new "watch <expression>: 监视（watch）一个python表达式"

    # renpy/common/00    new ""

    # renpy/common/00console.rpy:478
    old "unwatchall: stop watching all expressions"
    new "unwatch <expression>: 停止监视（watch）某个表达式"
...
translate simplified_chinese strings:
    ...
    new "unwatch <expression>:停止监视一个表达"
