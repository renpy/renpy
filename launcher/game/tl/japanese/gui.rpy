...
translate japanese strings:
    ...
    new "## スモールカラー。明るさを調節する必要のある、クイックメニューなどの小さなテキストボタンに使います。"
    ...
    new "## バーの非充足部分（右側）や、スライダーの背景部分のカラー。バーやスライダーのカラーは、ゲームでは直接使われず、GUIを変更・更新した場合の画像生成に使われます。"
    ...
    new "## メインメニューに、ゲームタイトルとそのバージョンを表示するかどうか。True なら表示します。"
    ...
    new "## キャラクター名を表示するネームボックスのサイズ。None にすると、自動的に決定されます。画像を変えたい場合は、gui/textbox.png の画像を入れ替えます（デフォルト画像は透明なので、表示されません）。"
    ...
    new "## 台詞の文字揃え。 0.0 は左揃え、0.5 は中央揃え、 1.0 は右揃えになります。0.0以外にした場合、台詞の位置の調整も必要になります。"
    ...
    new "## ボタンの縦幅と横幅。None にすると、自動的に計算されます。"
...
radio と check は環境設定の各項目のボタン（デフォルトでは同じ画像）。confirm は確認画面の選択肢、page は セーブ・ロード画面のページ切り替え、quick はクイックメニュー、 navigation はゲームメニューのメニュー切り替えに使うボタンです。"
    ...
    new "## デフォルトの GUI は、スライダーと垂直スクロールバーだけを使用します。他のバーは、開発者が追加したコードでのみ使われます。"
    ...
    new "## True に設定すると、バーの背景を、スケーリングではなくタイリングで表示します。"
    ...
    new "## ヒストリーのエントリー（１台詞）を、最大いくつまで保持するか。"
    ...
    new "## NVL モードにおいて、gui.nvl_height を None に設定した場合の、各エントリーの間隔。また、台詞と選択肢との間隔にも使われます。"
