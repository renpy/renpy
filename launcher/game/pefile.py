...
class PE:
    ...
    def __parse__(self, fname, data, fast_load):
        ...
        for i in xrange(int(0x7fffffffL & self.OPTIONAL_HEADER.NumberOfRvaAndSizes)):
            ...
            if offset >= (optional_header_offset +
                self.OPTIONAL_HEADER.sizeof() + 8 * 6) :
    ...
    def parse_directory_bound_imports(self, rva, size):
        ...
        while True:
            ...
            for idx in xrange(bnd_descr.NumberOfModuleForwarderRefs):
                ...
                if not name_str:
                    break
                forwarder_refs.append(BoundImportRefData(struct=bnd_frwd_ref,
                                        name=name_str))
            ...
            bound_imports.append(
                BoundImportDescData(struct=bnd_descr, name=name_str,
    ...
    def parse_relocations_directory(self, rva, size):
        ...
        while rva < end:
            ...
            relocations.append(
                BaseRelocationData(struct=rlc, entries=reloc_entries))
    ...
    def parse_relocations(self, data_rva, rva, size):
        ...
        for idx in xrange(len(data) / 2):
            ...
            entries.append(
                RelocationData(type=reloc_type, rva=reloc_offset + rva))
    ...
    def parse_resources_directory(self, rva, size=0, base_rva = None, level = 0):
        ...
        number_of_entries = (resource_dir.NumberOfNamedEntries +
        ...
        for idx in xrange(number_of_entries):
            ...
            if res.DataIsDirectory:
                ...
                if not entry_directory:
                    break
                dir_entries.append(
                    ResourceDirEntryData(struct=res, name=entry_name,
                                    id=entry_id, directory=entry_directory))
        ...
        resource_directory_data = ResourceDirData(struct=resource_dir,
                                                  entries=dir_entries)
