
# game/script.rpy:85
translate russian start_0e6a5bb4:

    # e "Hi! My name is Eileen, and I'd like to welcome you to the Ren'Py tutorial."
    e "Привет! Меня зовут Эйлин, и я хотела бы поприветствовать вас в обучающей программе Ren'Py."

# game/script.rpy:89
translate russian start_d3abb53c:

    # e "In this tutorial, we'll teach you the basics of Ren'Py, so you can make games of your own. We'll also demonstrate many features, so you can see what Ren'Py is capable of."
    e "Здесь мы обучим вас основам Ren'Py, чтобы вы могли сами создавать игры. Мы также продемонстрируем множество возможностей, чтобы вы могли видеть на что Ren'Py способен."

# game/script.rpy:118
translate russian end_b2482727:

    # e "Thank you for viewing this tutorial."
    e "Спасибо за просмотр обучения."

# game/script.rpy:120
translate russian end_38362e36:

    # e "If you'd like to see a full Ren'Py game, select \"The Question\" in the launcher."
    e "Если вы хотите увидеть полную Ren'Py-игру, выберите \"Вопрос\" в Launcher."

# game/script.rpy:122
translate russian end_894a9e66:

    # e "You can download new versions of Ren'Py from {a=http://www.renpy.org/}http://www.renpy.org/{/a}. For help and discussion, check out the {a=http://lemmasoft.renai.us/forums/}Lemma Soft Forums{/a}."
    e "Вы можете скачивать новые версии Ren'Py с {a=http://www.renpy.org/}http://www.renpy.org/{/a}. Для помощи и обсуждения, смотрите {a=http://lemmasoft.renai.us/forums/}форумы Lemma Soft{/a}."

# game/script.rpy:124
translate russian end_a898b369:

    # e "We'd like to thank Piroshki for contributing my sprites, Mugenjohncel for Lucy and the band, and Jake for the magic circle."
    e "Мы хотели бы поблагодарить Piroshki за мои изображения, Mugenjohncel за Люси и рок-группу, и Jake — за волшебный круг."

# game/script.rpy:126
translate russian end_762dc07a:

    # e "The background music is \"Sunflower Slow Drag\", by Scott Joplin and Scott Hayden, performed by the United States Marine Band. The concert music is by Alessio."
    e "Музыка, играющая на заднем фоне — \"Sunflower Slow Drag\", написанная Scott Joplin и Scott Hayden, и исполненная United States Marine Band. Музыка концерта написана Alessio."

# game/script.rpy:130
translate russian end_57c21073:

    # e "We look forward to seeing what you can make with Ren'Py. Have fun!"
    e "Мы хотим увидеть, что вы сделаете с Ren'Py. Веселитесь!"

translate russian strings:

    # game/script.rpy:18
    old "User Experience"
    new "Опыт пользователя"

    # game/script.rpy:18
    old "Writing Dialogue"
    new "Диалог"

    # game/script.rpy:18
    old "Adding Images"
    new "Добавление изображений"

    # game/script.rpy:18
    old "Music and Sound Effects"
    new "Музыка и звуковые эффекты"

    # game/script.rpy:18
    old "In-Game Menus and Python"
    new "Внутриигровые меню и Python"

    # game/script.rpy:18
    old "Screen Positions"
    new "Позиции экрана"

    # game/script.rpy:18
    old "Animation and Transformation"
    new "Анимации и трансформации"

    # game/script.rpy:18
    old "Video Playback"
    new "Воспроизведение видео"

    # game/script.rpy:18
    old "Transition Gallery"
    new "Галерея переходов"

    # game/script.rpy:18
    old "Image Operations"
    new "Операции над изображениями"

    # game/script.rpy:18
    old "User Interaction"
    new "Взаимодействие с пользователем"

    # game/script.rpy:18
    old "Fonts and Text Tags"
    new "Шрифты и теги"

    # game/script.rpy:18
    old "Character Objects"
    new "Объекты-персонажи"

    # game/script.rpy:18
    old "Layers & Advanced Show"
    new "Слои и продвинутое отображение"

    # game/script.rpy:18
    old "NVL Mode"
    new "Режим NVL"

    # game/script.rpy:18
    old "Dynamic Displayables"
    new "Динамические отображаемые объекты"

    # game/script.rpy:18
    old "Minigames"
    new "Мини-игры"

    # game/script.rpy:18
    old "Persistent Data"
    new "Постоянные данные"

    # game/script.rpy:18
    old "Transform"
    new "Трансформации"

    # game/script.rpy:18
    old "Sprites"
    new "Спрайты"

    # game/script.rpy:64
    old "That's enough for now."
    new "Этого достаточно."

    # game/script.rpy:99
    old "What would you like to see?"
    new "Что ещё вы хотели бы увидеть?"

    # game/script.rpy:101
    old "Is there anything else you'd like to see?"
    new "Хотите ли вы увидеть ещё что-то?"

